...
def _get_vocalizations(audio_paths,
                       spect_paths,
                       annot_paths,
                       annot_format,
                       ):
    ...
    if annot_paths is not None:
        if isinstance(annot_paths, pathlib.Path) or isinstance(annot_paths, str):  # just one annot file
    ...
    assert len(annot_paths) == len(audio_paths) == len(spect_paths)  # should be True by this point no matter what
...
def test_dataset(audio_format,
    ...
    audio_paths, spect_paths, annot_paths = paths_factory(audio_format,
                                                          spect_format,
                                                          annot_format,
                                                          path_as_str=path_as_str)

    vocalizations = _get_vocalizations(audio_paths,
                                       spect_paths,
                                       annot_paths,
                                       annot_format,
                                       )
    ...
    assert all([
        vocal in dataset.vocalizations for vocal in vocalizations
    ])
